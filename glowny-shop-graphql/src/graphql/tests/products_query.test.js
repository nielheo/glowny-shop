...
describe('viewer/products', () => {
  ...
  describe('select', function () {
    it('select products count is 2', function () {
      return test('{ "query": "{ viewer { products(shopCode:\\"glowny_shop\\") { id } } }" }')
      .then(result => {
        expect(result.status).to.equal(200)
        expect(result.success).to.equal(true)
        expect(result.data.viewer.roles.length).to.equal(2)
      })
    })
    ...
    it('select users first:1 - return all expected fields', function () {
      return test('{ "query": "{ viewer { products(shopCode:\\"glowny_shop\\") { id sku name description curr price } } }" }')
      .then(result => {
        expect(result.status).to.equal(200)
        expect(result.success).to.equal(true)
        ...
        expect(result.data.viewer.products[0].curr).to.not.be.empty()
        expect(result.data.viewer.products[0].price).to.not.be.empty()
      })
    })
  })
